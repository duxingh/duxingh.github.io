...
> 某点 $A$ 是集合 $S$ 的**聚点**是指，在任意接近 $A$ 的范围内都存在无穷多个 $S$ 的点。但是并未说 $A$ 属于集合 $S$。

显然，若 $a_n \to \alpha$，则 $\alpha$ 是数列 $\\{a_n\\}$ 的聚点。

> <strong id="theorem-9">定理 9</strong>（魏尔斯特拉斯定理） &emsp; 有界无穷点集必存在聚点。
{: .prompt-info }

证明：先证明一维的情形。设 $S$ 为一维有界无穷点集，则存在实数 $m_0, l_0$ 对任意的 $s \in S$ 满足 $m_0 \leq s \leq l_0$。令闭区间 $I_0 = [m_0,l_0]$；取 $m_0, l_0$ 之间的中点 $c = (m_0 + l_0)/2$，在 $[m_0, c], [c,l_0]$ 两者之中任意选择一个包含无穷个 $S$ 的点的闭区间，令其为 $I_1 = [m_1, l_1]$；取 $m_1, l_1$ 之间的中点 $c = (m_1 + l_1)/2$，在 $[m_1, c], [c,l_1]$ 两者之中任意选择一个包含无穷个 $S$ 的点的闭区间，令其为 $I_2 = [m_2, l_2]$；以此类推，得到一个闭区间套 $\\{I_n\\}$，且其长度 $\|I_n\| = (l_0 - m_0)/2^n$ 可无穷小。根据闭区间套法，可知存在唯一一个实数 $\alpha$ 满足 $\alpha \in I_n$。

对任意的正实数 $\varepsilon$，必然存在一个闭区间 $I_N = [m_N, l_N]$ 满足

$$
\alpha - \varepsilon < m_N \leq \alpha \leq l_N < \alpha + \varepsilon
$$

因为 $I_N$ 含有无穷个 $S$ 的点，所以有无穷个 $S$ 的点在 $\alpha$ 的邻域 $(\alpha - \varepsilon, \alpha + \varepsilon)$ 内。故 $\alpha$ 是 $S$ 的聚点。

再推广到多维情形。设 $S$ 为 $w$ 维有界无穷点集 $\\{(s_1, s_2, \cdots, s_w)\\}$，每一维度 $s_i$ 都是有界的。参考上面一维闭区间套的构造方法，我们可以构造一个 $w$ 维的“闭 $w$ 维体套” $\\{Q_n\\}$。同样应用闭区间套法，可知存在一个点 $(\alpha_1, \alpha_2, \cdots, \alpha_w) $，它属于每一个 $Q_n$。类似地，可以证明这个点就是 $S$ 的聚点。$\square$

> $S$ 的聚点不一定属于 $S$。如果 $S$ 的所有聚点都属于 $S$，则称 $S$ 为**闭集**。

> <strong id="theorem-10">定理 10</strong> &emsp; 设有界闭集列 $S_1, S_2, \cdots$ 满足：
>
> （1°）$S_1 \supset S_2 \supset \cdots \supset S_n \supset \cdots$，  
> （2°）当 $n$ 无限增大时，$S_n$ 的直径无限减小，
>
> 则这些集合 $S_n$ 存在唯一一个公共点。
{: .prompt-info }
...
证明：在 $S_n$ 中任取一点 $P_n$，由此构成一个点列 $\\{P_n\\}$。由条件（2°）可知，点列 $\\{P_n\\}$ 符合 Cauchy 条件，故存在点 $A$ 满足 $P_n \to A$。$A$ 便是 $\\{P_n\\}$ 的一个聚点。对于任意一个 $S_n$，由条件（1°）可知，$P_n, P_{n+1}, \cdots$ 都属于 $S_n$，所以 $A$ 便是 $S_n$ 的一个聚点。由 $S_n$ 为闭集可知 $A \in S_n$。故 $A$ 为 $\\{S_n\\}$ 的公共点。再由条件（2°）可知，$\\{S_n\\}$ 不可能拥有两个不同的公共点。$\square$

> <strong id="theorem-11">定理 11</strong>（Heine-Borel 覆盖定理） &emsp; 若有界闭集 $F$ 被一组无穷多个圆整体覆盖，则 $F$ 能被这些圆中的有限多个覆盖。
{: .prompt-info }

证明：假设该定理为假。取包围 $F$ 的一个 $w$ 维体 $Q$，将 $Q$ 的每一个维度都对半分，由此得到 $2w$ 个小的 $w$ 维体。取其中一个（边界也考虑在内）与 $F$ 的交集为 $F_1$，则显然 $F_1$ 为闭集。对于 $F_1$ 来说，该定理也为假；否则，该定理将对 $F$ 为真。对 $F_1$ 重复类似操作，便得到一个有界闭集列 $\\{F_n\\}$，满足 $F_1 \supset F_2 \supset \cdots \supset F_n \supset \cdots$，且 $F_n$ 的直径无限减小。根据定理 10，可知存在 $F_n$ 的唯一一个公共点 $\alpha$。可是显然，一个点如果被无穷个圆覆盖，必然能从这些圆中取出一个覆盖此点；而对于充分大的 $n$，$F_n$ 必然能被包含在这个圆内，即 $F_n$ 被这个圆覆盖。这一矛盾便证明了此定理为真。$\square$
